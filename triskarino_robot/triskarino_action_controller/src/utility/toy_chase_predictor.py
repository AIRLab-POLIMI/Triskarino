...
class ToyChaseTunablePredictor(Predictor):
  ...
  #rotation_unknown_starter: how much should the rotation start if the ball is not seen
    ...
    if type == "toy":
      ...
      if x[1] == -1:
        ...
        if self.last_valid_angle == None:
          ...
          #Negative sign because to rotate right (positive angle) robot has to receive a negative cmd_vel
          rot_prediction = -self.param_choice["rotation_unknown_starter"] * self.last_valid_angle
      else:
      ...
      angle = get_bbox_angle(x, type="stick")
      self.last_valid_angle = angle
      rot_prediction = self._get_rot_prediction(angle)
      rot_prediction = cap_number(rot_prediction,self.min_arr[2], self.max_arr[2])
      #TODO: Figure out if for linear prediction we can do something better than this
      linear_prediction = 0
      self.last_linear_movement = linear_prediction
      self.last_angular_movement = rot_prediction
      return [linear_prediction, 0, rot_prediction]      
