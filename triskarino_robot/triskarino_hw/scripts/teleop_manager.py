...
INPUT_PATH = "/dev/input/by-id/usb-Logitech_Wireless_Gamepad_F710_FD149CD6-event-joystick"
...
class TeleopManagerNode():
    """
    TeleopManagerNode is responsible for managing teleoperation inputs from a gamepad and publishing corresponding
    ROS messages to control a robot's movement, lights, and sounds. Certain global variables like:
    constants for the light, for the volume and max velocities are set in the script. To make it work, modifying the input 
    path of the gamepad with your specific one is needed.
    Attributes:
        NODE_NAME (str): Name of the ROS node.
        BRIGHTNESS (int): Default brightness level for the lights.
    Methods:
        __init__():
            Initializes the TeleopManagerNode, setting up publishers, gamepad input device, and dynamic reconfigure server.
        reconfigure_callback(config, level):
            Callback function for dynamic reconfigure server to update delay and volume parameters.
        get_teleop_input():
            Reads inputs from the gamepad and publishes corresponding ROS messages for robot control.
    """
    ...
    def __init__(self):
        ...
        self.cmd_pub = rospy.Publisher('cmd_autonomous_joy',String,queue_size=PUBLISHER_QUEUE_SIZE,latch=True)
    ...
    def reconfigure_callback(self,config,level):
        ...
        self.volume_fear = config.volume_fear
        self.volume_joy = config.volume_joy
        self.volume_sad = config.volume_sad
        self.volume_angry = config.volume_angry
        self.volume_disgust = config.volume_disgust
    ...
    def get_teleop_input(self):
        ...
        for event in self.gamepad.read_loop():
            ...
            ##################### Lights #####################
                ...
                #BUTTON A: green light disgust
                light_msg.action = "A"               # colorWipe: wipe color across display
                light_msg.color = [14, 123, 15]        # Green
            ...
            ##################### Commands #####################
                ...
                cmd_msg.data = "on" if self.chase_toy else "off"
