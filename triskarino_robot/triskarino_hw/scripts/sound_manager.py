...
class SoundManagerNode():
    """
    A ROS node that manages sound playback.
    Attributes:
        NODE_NAME (str): The name of the ROS node.
        sound_subscriber (rospy.Subscriber): Subscriber to the "sound" topic.
    Methods:
        __init__():
            Initializes the SoundManagerNode, sets up the ROS node, and subscribes to the "sound" topic.
        play_sound(sound_data):
            Callback function that plays the sound specified in the sound_data message using the play command from terminal. The message on the sound topic contains the filepath of the sound.
            Logs the current working directory and its contents before executing the sound playback command.
    """
