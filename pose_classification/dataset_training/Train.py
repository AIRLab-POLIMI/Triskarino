...
def print_class_distribution(df, set_name, total_class_counts):
    ...
    for class_name, percentage in class_counts.items():
        ...
        print(f"{class_name}: {percentage:.2f}% of the set ({class_percentage_in_set:.2f}% of total for this class) ")
...
def split_dataset(file_path, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15):
    ...
    df['class'] = df['video_id'].str.extract(r'(^[A-Za-z]+)')
    
    # Map to numeric classes and drop others
    df['class'] = df['class'].map({
        'Standing': 0, 'Sitting': 1, 'SittingInChair': 2, 'Crawling': 3
    })   
    df = df.dropna(subset=['class'])  # Drop rows from unused classes

    # Group by video_id to ensure sequence consistency
    video_groups = df.groupby('video_id').first().reset_index()
    
    # Stratify split based on class while keeping video_id grouped
    train_videos, temp_videos = train_test_split(video_groups, test_size=(1 - train_ratio), stratify=video_groups['class'], random_state=42)
    val_videos, test_videos = train_test_split(temp_videos, test_size=(test_ratio / (test_ratio + val_ratio)), stratify=temp_videos['class'], random_state=42)
    
    # Filter the original dataframe based on selected video_ids
    train_set = df[df['video_id'].isin(train_videos['video_id'])]
    val_set = df[df['video_id'].isin(val_videos['video_id'])]
    test_set = df[df['video_id'].isin(test_videos['video_id'])]
    
    # Print final split percentages
    print(f"Final Split Percentages:")
    print(f"Training: {len(train_set) / len(df):.2%}")
    print(f"Validation: {len(val_set) / len(df):.2%}")
    print(f"Testing: {len(test_set) / len(df):.2%}")
...
def plot_confusion_matrix_with_info(cm, class_names, train_set, val_set, test_set, total_class_counts, df, y_true, y_pred, model, model_filename):
    ...
    plt.gcf().text(1.3, 0.5, info_text, fontsize=10, verticalalignment='center', horizontalalignment='left',
    ...
    plt.gcf().text(0.5, 0.01, param_summary, fontsize=10, verticalalignment='bottom', horizontalalignment='center',
                   bbox=dict(boxstyle="round", facecolor="white", alpha=0.8), transform=fig.transFigure)
    
    plt.show()

train_set, val_set, test_set, df = split_dataset(file_path)
...
selected_features = angle_features #+ custom_features + euc_features
...
model_filename = "model_weights_onlyangles_minimal.xgb"
# Save the trained model
model.save_model("/input/your/save/path/" + model_filename)

# Compute confusion matrix
cm = confusion_matrix(y_test, y_pred)

# Call the function to plot confusion matrix
plot_confusion_matrix_with_info(cm, class_names, train_set, val_set, test_set, df['class'].value_counts(), df, y_test, y_pred, model, model_filename)
